...
\label{sec:ov-security-analysis}
  ...
  \begin{corollary}[\fpaynet{} provides Balance Security]
    ...
    \end{equation*}
  \end{corollary}
  where $\mathsf{BAL \mhyphen SEC}$ is defined below. % TODO: fix if above
  ...
  \begin{gamebox}{$\mathsf{BAL \mhyphen SEC}^{\adversary}\left(1^k\right)$}
    ...
    \begin{algorithmic}[1]
      \Procedure{channelLifecycle}{}
        ...
        \Until{$\exists (\alice: c_0, \bob: 0, \mathit{pchid}) \in
        \mathtt{newChannels}$}
        ...
        \For{$i$ from $1$ to an arbitrary $n_{\mathrm{pays}} \in \mathbb{N}$}
          ...
          \Else
            \State \Comment{\TODO{figure out how to keep track of incoming}}
