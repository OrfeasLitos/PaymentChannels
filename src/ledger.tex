\section{The Ledger Functionality and its Properties}
  ...
  \begin{enumerate}
    \item in a period of time equal to \maxTime, at least \windowSize blocks are
    added to \state.
    ...
    \windowSize positions before the head of the \state must be included in an
    honest block.
  \end{enumerate}
  ...
  \begin{proposition}
    ...
    and $h$ an arbitrary positive integer. Then, as long as $t\geq ???$, \tx is
    guaranteed to be included in the correct block range as long as the party
