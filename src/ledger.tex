\section{The Ledger Functionality and its Properties}
  ...
  The key characteristics of the functionality are as follows. A variable
  $\state$ maintains the current immutable state of the ledger. Honest parties
  that are synchronised may find themselves  to consider finalised various
  prefixes of $\state$ (specified by a pointer position $\pt_{i}$  for party
  $\stkh_i$ below) but nevertheless no such prefix will be shorter than a
  parameter $\windowSize$ from $\state$. On any input originating from an honest
  party the functionality will run the $\LFextend$ function that will ensure
  that a suitable sequence of transactions will be ``blockified'' and added to
  ...
  resources that are necessary for proper ledger maintenance and the
  functionality keeps track of are the global random oracle $\Foracle$, the
  clock $\Fclock$ and network $\FnetworkEd$. When an honest party maintains
  registration with all the resources then after $\vdelay$ clock ticks it must
  necessarily become synchronised.
  ...
  \begin{enumerate}
    ...
    are added to  $\state$.
    \item each window of $\windowSize$ blocks should have a limit of
    $\advBlocksinWindowSize$ of adversarial blocks included in it.
    \item any transaction transmitted by an honest party earlier than
  ...
  \begin{proposition}
    ...
    submits $\tx$ to the ledger functionality no later than $????$.
  \end{proposition}
