...
\label{sec:ov-protocol}
  ...
  \redden{fulfills the security} guarantees given by \fpaynet, a concrete
  protocol that implements LN in the UC model \redden{is needed}. To that end we
  ...
  \charlie{} \redden{are interactive Turing machine instances (ITIs)~\cite{uc}
  ...
  \noindent {\bf Channel opening.}
    ...
    channel with \bob, funded by her with $x$ coins. \redden{After following the
    ...
    that case she \redden{exchanges} new commitment key\redden{s with \bob, as
  ...
  \noindent {\bf Channel closing.}
    When \redden{sent by \environment, the messages \textsc{closeChannel} and
  ...
  \noindent {\bf Performing payments.}
    \redden{We will now follow the exact steps needed for a multi-hop payment,
    ...
    \redden{``}invoice\redden{''} with the HTLC hash, to which \charlie{} reacts
    ...
    sanity checks \redden{and extracting the hash}, he takes a note of this
    ...
    subsequently receives (\textsc{commit}, $\mathit{pchid}_{\redden{AB}}$) from
    \environment, where $\mathit{pchid}_{\redden{AB}}$ is the ID of the \alice{}
    ...
    that before \bob{} forwards the onion \redden{to \charlie}, he \redden{must}
    commit \redden{as well} to the HTLC to \alice. This happens if he receives
    \redden{the relevant} \textsc{commit} message from \environment. Now that both
    ...
    \bob{} may then receive (\textsc{pushAdd}, $\mathit{pchid}_{\redden{AB}}$)
    ...
    $\mathit{pchid}_{\redden{BC}}$) message from \environment. If it arrives,
    ...
    channels it participates \redden{in} in order to remove the HTLC and update
    ...
    (\textsc{getNews}). \redden{\alice{} and \charlie{} also keep a note to
...
%
    ...
    it actually implements an IBS, see \redden{Appendix}~\ref{sec:ibs}.
    ...
    roles. Furthermore, in order to reduce transaction size \redden{and ensure
