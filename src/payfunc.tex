        ...
        \Indent
          \ForAll{$\charlie \text{ keys} \in \texttt{charged}$}
            ...
            \mathtt{charged}\left(\charlie\right)$}
              ...
              \If{$\dave{} \neq \alice{} \wedge \dave{} \notin
              \mathtt{corrupted} \ \wedge ((\Sigma_{\dave}$ contains in block
              $h_{\mathtt{tx}}$ an old $\mathtt{remoteCom}_m$ that does not
              contain the HTLC and a tx that spends the delayed output of
              $\mathtt{remoteCom}_m \ \wedge$ \texttt{polls}(\dave) contains an
              element in $[h_{\mathtt{tx}} + k, h_{\mathtt{tx}} + k +
              \mathtt{delay}\left(\dave\right) - 1]) \ \vee$ ($\Sigma_{\dave}$
              does not contain such a $\mathtt{remoteCom}_m \ \wedge$
              ...
              \label{alg:fpaynet:resolvepay:haltcond}
                \State halt
                \label{alg:fpaynet:resolvepay:halt}
              \EndIf
    ...
    \begin{systembox}{\fpaynet - close}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \State do not serve any other (\textsc{pay}) message for this channel
          \State send (\textsc{closeChannel}, \texttt{receipt}, \alice) to
          \simulator
        \EndIndent
        \Statex

        \Function{checkClosed}{$\Sigma_{\alice}$} \Comment{Ensures requested
        closes eventually happen}
          \ForAll{entries $(\mathit{fchid}, \mathtt{receipt}, h) \in
          \mathtt{pendingClose}(\alice)$}
            ...
            corresponds to \texttt{receipt}}
              ...
              \EndIf
            \ElsIf{there is a closing transaction in $\Sigma_{\alice}$ for open
            \texttt{channel} with ID \textit{fchid} with a balance that does not
            correspond to the \texttt{receipt}}
              \State halt
    ...
    \begin{systembox}{\fpaynet - poll}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \ForAll{$\mathtt{channel}s \in \Sigma_{\alice}$ that contain alice and
          are maliciously closed by a remote commitment \texttt{tx} (one with an
          older channel version than the irrevocably committed one) in block
          with height $h_{\mathtt{tx}}$}
