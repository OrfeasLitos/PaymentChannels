    ...
    \begin{systembox}{\fpaynet{} -- interface}
      \begin{itemize}
        ...
        \begin{itemize}
          ...
          \texttt{updatesToReport}, \texttt{paymentsToReport})
        \end{itemize}
        \item from $\simulator$:
        \begin{itemize}
        ...
        \begin{itemize}
          ...
          \item (\textsc{adversaryOpenChannel}, \textit{fchid}, \textit{intid})
    ...
    \begin{systembox}{\fpaynet{} -- pay}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \mathit{expayid}, \mathit{payid}\right)$ to \texttt{pendingPay}
        ...
        \State Upon receiving (\textsc{newUpdate}, \texttt{receipt}, \alice)
        from \simulator:
        \label{alg:fpaynet:update}
        \Indent
          \State append \texttt{receipt} to \texttt{updatesToReport}(\alice)
        ...
        \State Upon receiving (\textsc{newPayments}, \texttt{payments}, \alice)
        from \simulator:
        ...
        \Indent
          ...
          add them to \texttt{paymentsToReport}
          \State send (\textsc{continue}) to \simulator
        \EndIndent
      \end{algorithmic}
    \end{systembox}
      ...
      \begin{algorithmic}[1]
        ...
        \Indent
          \ForAll{$\alice \text{ keys} \in \texttt{charged}$}
            ...
            ?\overrightarrow{\mathtt{path}}\right) \in
            \mathtt{charged}\left(\alice\right)$}
              ...
              \If{there is a $\overrightarrow{\mathtt{path}}$ in neither or both
              the \texttt{pendingPay} and \texttt{charged} entries}
                \State halt
              \EndIf
              \If{$\dave{} = \bot$} \Comment{Payment failed}
              \label{alg:fpaynet:resolvepay:failed:if}
                \If{there is an (\textit{expayid}, \textit{payid}, \_) entry in
                \texttt{pendingDiffs}(\alice) or \texttt{pendingDiffs}(\bob) AND
                \alice{} or \bob{} respectively is honest}
    ...
    check}{roundbox}{normal}
      \begin{algorithmic}[1]
        ...
        \label{alg:fpaynet:resolvepay:loops:halt:loop}
          \State ensure that an element reporting the new balance exists either
          in the \texttt{updatesToReport} or the \texttt{updatesHistory} of each
          honest party and that element has not been checked by the current line
          in the past, otherwise halt
          \label{alg:fpaynet:resolvepay:loops:halt:do}
        \EndFor
        \If{$\dave{} = \alice{} \wedge{} \alice{} \notin \mathtt{corrupted}
        \wedge (\mathit{expayid}, \mathit{payid}, -x) \notin
        ...
        \label{alg:fpaynet:close:func:start}
          ...
          \mathtt{pendingClose}(\alice) \cup \mathtt{closedChannels}(\alice)$}
            ...
            \label{alg:fpaynet:close:func:happy:start}
              ...
              \State remove \texttt{channel} from \texttt{channels} and entry
    ...
    \begin{systembox}{\fpaynet{} -- daemon messages}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \texttt{paymentsToReport} and send them to \alice{} with message name
          \textsc{news}, stripping \textit{fchid} and h from
          \texttt{closedChannels}(\alice)
