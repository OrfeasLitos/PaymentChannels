  ...
  \begin{figure}[H]
    \begin{systembox}{\fpaynet{} -- pay}
      \begin{algorithmic}[1]
        ...
        \Indent
          \State let \bob{} be \alice's counterparty in the channel referred to
          by \texttt{receipt}
          % TODO: 1) modify proof to pass this test
          %       2) maybe should accept diffs bigger than intended?
          %       3) fix, only covers if both honest
          %       4) if possible move checks to resolvePays for better locality
          %       of proof
          \State ensure \texttt{diffs} is a set of $(\mathit{payid}, x)$ pairs
          such that for each pair $x$ is non-zero and, if $x > 0$, we have
          previously received (\textsc{pay}, \alice, $x$, \_, \_) through \bob,
          else if $x < 0$, we have previously received (\textsc{pay}, \bob,
          $-x$, \_, \_) through \alice, otherwise halt
