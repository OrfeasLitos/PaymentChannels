  ...
  \begin{figure}[H]
    \begin{systembox}{\fpaynet - resolve payments}
      \begin{algorithmic}[1]
        ...
        \Comment{from \textsc{resolvePay}, \textsc{channelsClosed}, from
        \textsc{pushFulfill}, from \textsc{pushAdd}, from \textsc{commit}}
        \Indent
          \ForAll{$\charlie \text{ keys} \in \texttt{charged}$}
            ...
            \mathtt{charged}\left(\charlie\right)$}
              ...
              1, \mathtt{IncomingCltvExpiry} - \TODO{2?}k - \tochain{} -
              1\right] \wedge$ \texttt{focs}(\dave) contains
    ...
    \begin{systembox}{\fpaynet - poll}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \textit{reportid}, \textsc{state}, $\Sigma$) to \simulator
        \EndIndent
      \end{algorithmic}
    \end{systembox}
    \caption{}
