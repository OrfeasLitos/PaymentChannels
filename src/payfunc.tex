    ...
    \begin{systembox}{\fpaynet{} - open}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \textsc{channelOpened}''
          \If{\bob{} is \texttt{corrupted}}
            \State send (\textsc{fundingLocked}, \alice, $\Sigma_{\alice}$,
            \textit{fchid}) to \simulator
            \label{alg:fpaynet:checkForNew:sim:cor}
          \Else
            \State send (\textsc{fundingLocked}, \alice, $\Sigma_{\alice},
            \Sigma_{\bob}$, \textit{fchid}) to \simulator
            \label{alg:fpaynet:checkForNew:sim:hon}
          \EndIf
        \EndIndent
        \Statex
        ...
        \Indent
          ...
          \label{alg:fpaynet:checkForNew:report}
