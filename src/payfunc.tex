\begin{functionality}{$\mathcal{F}_{\mathrm{PayNet}}$}
  ...
  \begin{algorithmic}[1]
    ...
    \State Upon receiving $\left(\textsc{createChannel}, Alice, Bob, x,
    \mathrm{forFundTX}\right)$ from $Alice$:
    \Indent
      \State ensure that forFundTX has inputs spendable by $Alice$ of total
      value at least $x$
      \State ensure that it is possible to add a $x$-valued output to forFundTX
      ...
      reply to $\tau$
      \State send $\left(\textsc{openChannel}, Alice, Bob, x, 0,
      ...
      \State $\mathtt{receipt} \gets \left(Alice, Bob, x, 0, 0,
      \mathrm{chid}\right)$
    ...
    \Indent
      ...
      \State condition 1: reply is \textsc{okPay} \Comment{TODO: maybe
      unnecessary}
      ...
      \If{conditions 1, 2 and 3 all hold}
