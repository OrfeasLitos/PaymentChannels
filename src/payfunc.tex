    ...
    \begin{systembox}{\fpaynet - support}
      ...
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \Comment{Must check chain at least once every \textt{delay}(\alice)
          blocks}
    ...
    \begin{systembox}{\fpaynet - pay}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \mathtt{corrupted} \vee$ \texttt{charged} has not been negligent from
          (the moment the previous party on the $\overrightarrow{\mathtt{path}}$
          irrevocably committed to the HTLC) + \texttt{delay} until
          $\mathtt{cltvExpiry} - 2k - 1 \vee$ (\texttt{charged} has received
          \textsc{poll} between $\mathtt{cltvExpiry} - \mathtt{relayDelay} - 2k$
          and $\mathtt{cltvExpiry} - 2k - 1$ inclusive $\wedge$ \texttt{charged}
          has received \textsc{fulfillOnChain} after the \textsc{poll} and
          $\mathtt{cltvExpiry} - k$)}
            \State halt
          \EndIf
