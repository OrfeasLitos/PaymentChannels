\section{Payment Network Functionality}
  ...
  channel and it forwards the request to \simulator. Since the closing of a
  channel is an event that can be observed on-chain, \fpaynet{} does not need to
  be informed by \simulator{} on whether its request has been carried out, but
  directly checks on-chain with \texttt{checkClosed}()
