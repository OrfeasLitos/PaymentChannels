\begin{functionality}{\fpaynet}
  ...
  \begin{algorithmic}[1]
    ...
    \Indent
      ...
      \State $\mathtt{negligent}\left(\alice\right) \gets \emptyset$
    ...
    \Indent
      ...
      \State send (\textsc{read}) to \clock{} as \alice and assign reply to
      $\tau$
      \If{$\mathrm{deadline} < \tau$}
        \State add $\left[\mathrm{deadline}, \mathrm{deadline} + \tau -
        1\right]$ to \texttt{negligent}(\alice)
      \EndIf
      \State send (\textsc{read}) to \ledger{} as \alice{} and assign reply to
      $\Sigma_{\alice}$
      ...
      \alice{} and exist in \texttt{channels} (spends of funding txs that have
      an older channel version than stored), send (\textsc{closeChannel},
      \texttt{channel}) to \simulator, ensure reply is (\textsc{channelClosed},
      \texttt{channel}), ensure that corresponding txs exist on-chain
      \orfeas{TODO}, remove them from \texttt{channels} and add their receipts
      to \texttt{closedChannels}(\alice)
      \State clear \texttt{newChannels}(\alice),
      \texttt{closedChannels}(\alice), texttt{pendingUpdates}(\alice) and send
      them to \alice
    \EndIndent
    \State
