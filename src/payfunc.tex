        ...
        \Indent
          \ForAll{$\charlie \text{ keys} \in \texttt{charged}$}
            ...
            \mathtt{charged}\left(\charlie\right)$}
              ...
              \If{$\dave{} \neq \alice{} \wedge \dave{} \notin
              \mathtt{corrupted} \ \wedge$ \texttt{polls}(\dave) contains an
              element in $\left[\mathtt{OutgoingCltvExpiry} + k + \tochain{} +
              1,\right.$ $\left.\mathtt{IncomingCltvExpiry} - k -
              \tochain{}\right] \wedge$ \texttt{focs}(\dave) contains
              $\mathtt{IncomingCltvExpiry} - k - \tochain{} \ \wedge$ the
              element in \texttt{polls}(\dave) was added before the element in
              \texttt{focs}(\dave)}
                \State halt
              \EndIf
