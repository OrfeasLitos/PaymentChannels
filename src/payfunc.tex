            ...
            \mathtt{charged}\left(\alice\right)$}
              ...
              \Else \ \Comment{\dave{} honest}
                ...
                \ElsIf{$\dave{} \neq \alice$}
                  ...
                  $\mathtt{remoteCom}_m$}
                    \If{\texttt{polls}(\dave) contains two elements in
                    $[\mathtt{OutgoingCltvExpiry}, \mathtt{IncomingCltvExpiry} -
                    \tochain{}]$ that have a difference of at least \tochain{} \
                    $\wedge$ \texttt{focs}(\dave) contains
                    $\mathtt{IncomingCltvExpiry} - \tochain{} \ \wedge$ \\ the
                    element in \texttt{polls}(\dave) was added before the
                    element in \texttt{focs}(\dave)}
