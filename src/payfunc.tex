\section{Payment Network Functionality}
  ...
  \begin{figure}[H]
  % TODO: missing relayDelay enforcement
          ...
          \ForAll{$\alice \text{ keys} \in \texttt{charged}$}
            ...
            \mathtt{charged}\left(\alice\right)$}
              ...
              \Else \ \Comment{\dave{} honest}
                ...
                \wedge$ \\ \texttt{polls}(\dave) contains an element in
                $[h_{\mathtt{tx}}, h_{\mathtt{tx}} +
                \mathtt{delay}\left(\dave\right) - 1]$}
                \label{alg:fpaynet:resolvepay:haltcond:rel}
                  \State halt \Comment{\dave{} \textsc{poll}ed, but malicious
                  closure}
                ...
                \ElsIf{$\dave{} \neq \alice$}
                  ...
                  two elements in $[\mathtt{OutgoingCltvExpiry},
                  \mathtt{IncomingCltvExpiry} - \tochain{}]$ that have a
                  difference of at least $\tochain{} \ \wedge$ \\
                  \texttt{focs}(\dave) contains $\mathtt{IncomingCltvExpiry} -
                  \tochain{} \ \wedge$ \\ the element in \texttt{polls}(\dave)
                  was added before the element in \texttt{focs}(\dave)}
        ...
        \label{alg:fpaynet:close:func:start}
          ...
          \mathtt{pendingClose}(\alice) \cup \mathtt{closedChannels}(\alice)$}
            ...
            corresponds to \texttt{receipt}}
              ...
              \EndIf
            \ElsIf{there is a tx in $\Sigma_{\alice}$ that is not a commitment
            tx and spends the funding tx of the \texttt{channel} with ID
            \textit{fchid}}
            \label{alg:fpaynet:close:func:ifdsforgery}
              \State halt \Comment{DS forgery}
              \label{alg:fpaynet:close:func:dsforgery}
            \ElsIf{there is a closing commitment transaction in block of height
            $h$ in $\Sigma_{\alice}$ for open \texttt{channel} with ID
            \textit{fchid} with a balance that does not correspond to the
            \texttt{receipt} and the delayed output has been spent by the
            counterparty and \texttt{polls}(\alice) contains an entry in $[h, h
            ...
            \EndIf
          \EndFor
          \If{\alice{} has no open channels in $\Sigma_{\alice}$ AND \alice{}
          has always \textsc{poll}ed according to the needs of
          Fig.~\ref{alg:fpaynet:close:func},
          line~\ref{alg:fpaynet:close:func:ifmalicious},
          Fig.~\ref{alg:fpaynet:poll}, line~\ref{alg:fpaynet:poll:malicious} and
          Fig.~\ref{alg:fpaynet:resolvepay},
          lines~\ref{alg:fpaynet:resolvepay:haltcond:rel}
          and~\ref{alg:fpaynet:resolvepay:haltcond:abs}}
      ...
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \label{alg:fpaynet:poll:malicious}
            ...
            \texttt{polls}(\alice) has an element in $\left[h_{\mathtt{tx}},
            h_{\mathtt{tx}} + \mathtt{delay}\left(\alice\right) - 1\right]$}
