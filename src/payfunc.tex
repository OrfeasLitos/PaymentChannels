    ...
    \begin{systembox}{\fpaynet{} - interface}
      \begin{itemize}
        ...
        \begin{itemize}
          ...
          \item (\textsc{forceCloseChannel}, \texttt{receipt}, \textit{tid})
          \item (\textsc{poll})
  ...
  \begin{figure}[H]
    \begin{systembox}{\fpaynet{} - close}
      \begin{algorithmic}[1]
        ...
        \textit{tid}) from \alice
        \Indent
          \State follow the same steps as in \textsc{closeChannel}, but instead
          of $\infty$ put $\bot$ in third slot of new element to
          \texttt{pendingClose}(\alice) and send \textsc{forceCloseChannel} to
          \simulator. Also do not serve any other (\textsc{pay},
          \textsc{closeChannel} or \textsc{forceCloseChannel}) message from
          \alice{} for this channel.
        \EndIndent
        \Statex
