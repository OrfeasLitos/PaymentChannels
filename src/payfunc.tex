\section{Payment Network Functionality}
    ...
    \begin{systembox}{\fpaynet{} -- interface}
      \begin{itemize}
        ...
        \begin{itemize}
          ...
          \item (\textsc{news}, \redden{\texttt{newChannels},
        ...
        \begin{itemize}
          ...
          \item (\redden{\textsc{newUpdate}}, \texttt{receipt}, \alice)
          \item \redden{(\textsc{newPayments}, \texttt{payments}, \alice)}
          \item (\textsc{closedChannel}, \texttt{channel}, \alice)
          \item (\textsc{resolvePays}, \texttt{charged})
          \item \redden{(\textsc{adversaryOpenChannel}, $x$, \texttt{bobDelay},
          \textit{tid}, \textit{intid}, \textit{from} \alice, \textit{to} \bob)}
          \item \redden{(\textsc{adversarySendInvoice}, $x$, \textit{expayid},
        ...
        \begin{itemize}
          ...
          \overrightarrow{\mathtt{path}}$, \redden{\textit{expayid}},
          \textit{payid}, \redden{\textsc{state}, $\Sigma$})
          \item (\textsc{continue})
          ...
          \item \redden{(\textsc{adversaryOpenChannel}, \textit{fchid},
          \textit{intid})}
          \item \redden{(\textsc{adversarySendInvoice}, \textit{invid})}
        \end{itemize}
      \end{itemize}
    \end{systembox}
    \caption{}
  ...
  \begin{figure}[H]
    \begin{systembox}{\fpaynet{} -- \redden{initiate} open}
      \begin{algorithmic}[1]
        ...
        \redden{\State Upon receiving (\textsc{adversaryOpenChannel}, $x$,
  ...
  \begin{figure}[H]
    \begin{systembox}{\fpaynet{} -- \redden{negotiate open}}
      \begin{algorithmic}[1]
  ...
  \begin{figure}[H]
    \begin{systembox}{\fpaynet{} -- pay}
      \begin{algorithmic}[1]
        ...
        \overrightarrow{\mathtt{path}}, \redden{\mathit{expayid}}\right)$ from
        \alice:
        \Indent
          \State choose unique payment ID \textit{payid}
          \State add $\left(\alice, \bob, x, \overrightarrow{\mathtt{path}},
          \redden{\mathit{expayid}}, \mathit{payid}\right)$ to
          \texttt{pendingPay}
        ...
        \redden{\State Upon receiving (\textsc{adversarySendInvoice}, $x$,
        ...
        \State Upon receiving (\redden{\textsc{newUpdate}}, \texttt{receipt},
        \alice) from \simulator:
        \label{alg:fpaynet:update}
        \Indent
          \State \redden{append} \texttt{receipt} to
        ...
        \redden{\State Upon receiving (\textsc{newPayments}, \texttt{payments},
        \alice) from \simulator:
      ...
      \begin{algorithmic}[1]
        ...
        \Indent
          \ForAll{$\alice \text{ keys} \in \texttt{charged}$}
            \ForAll{$\left(\dave, \mathit{payid},
            \redden{?\overrightarrow{\mathtt{path}}}\right) \in
            \mathtt{charged}\left(\alice\right)$}
              ...
              \redden{\If{there is a $\overrightarrow{\mathtt{path}}$ in neither
              or both the \texttt{pendingPay} and \texttt{charged} entries}
                \State halt
              \EndIf}
              \If{$\dave{} = \bot$} \Comment{Payment failed}
              \label{alg:fpaynet:resolvepay:failed:if}
                \redden{\If{there is an (\textit{expayid}, \textit{payid}, \_)
                entry in \texttt{pendingDiffs}(\alice) or
                \texttt{pendingDiffs}(\bob) AND \alice{} or \bob{} respectively
                is honest}
              ...
              \ElsIf{$\dave{} \in \mathtt{corrupted}$}
                ...
                \State \redden{increase \texttt{offChainBalance}(\bob) by $x$}
              ...
              \EndIf
            \EndFor
            % TODO: verify that there are no negative elements in
            % pendingDiffs(\alice) that pertain to open channels, halt otherwise
          \EndFor
        \EndIndent
      \end{algorithmic}
    \end{systembox}
  ...
  \begin{figure}[H]
    \begin{titlebox}{\normalfont Honest payer}{roundbox}{normal}
      \begin{algorithmic}[1]
        ...
        \If{$\Sigma_{\dave}$ contains a tx that is not a $\mathtt{localCom}_n$
        or a $\mathtt{remoteCom}_n$ and spends a funding tx for an open
        \texttt{channel} that contains \dave{}}
        ...
        \ElsIf{$\Sigma_{\dave}$ contains in block $h_{\mathtt{tx}}$ an old
        $\mathtt{remoteCom}_m$ that does not contain the HTLC and a tx that
        spends the delayed output of $\mathtt{remoteCom}_m$}
          \If{\texttt{polls}(\dave) contains an element in $[h_{\mathtt{tx}},
          h_{\mathtt{tx}} + \mathtt{delay}\left(\dave\right) - 1]$}
        ...
        \ElsIf{$\dave{} \neq \alice$}
          ...
          \If{$\Sigma_{\dave}$ does not contain an old $\mathtt{remoteCom}_m$}
            \If{failure condition of
            Fig.~\ref{alg:fpaynet:resolvepay:haltcond:abs} is true}
              \State halt \Comment{\dave{} \textsc{poll}ed and fulfilled, but
              charged}
        ...
        \State \redden{decrease \texttt{offChainBalance}(\dave) by $x$}
    ...
    check}{roundbox}{normal}
      \begin{algorithmic}[1]
        ...
        \label{alg:fpaynet:resolvepay:loops:halt:loop}
          \State ensure that \redden{an element reporting the new balance exists
          either in the \texttt{updatesToReport} or the \texttt{updatesHistory}}
          of each honest party \redden{and that element has not been checked by
          the current line in the past}, otherwise halt
          \label{alg:fpaynet:resolvepay:loops:halt:do}
        \EndFor
        \redden{\If{$\dave{} = \alice{} \wedge{} \alice{} \notin
        \mathtt{corrupted} \wedge (\mathit{expayid}, \mathit{payid}, -x) \notin
        \mathtt{pendingDiffs}(\alice)$} \Comment{payer not informed}
        ...
        \If{$\bob{} \notin \mathtt{corrupted} \wedge (\mathit{expayid},
        \mathit{payid}, x) \notin \mathtt{pendingDiffs}(\bob)$} \Comment{payee
        not informed}
        \label{alg:fpaynet:resolvepay:loops:indiffs:credit:cond}
          \State halt
          \label{alg:fpaynet:resolvepay:loops:indiffs:credit:halt}
        \EndIf}
        \State \redden{remove (\textit{expayid}, \textit{payid}, $x$) from
  ...
  % TODO: verify that all payments are already reported, halt otherwise
  \begin{figure}[H]
        ...
        \label{alg:fpaynet:close:func:start}
          ...
          \mathtt{pendingClose}(\alice) \cup \mathtt{closedChannels}(\alice)$}
            ...
            \label{alg:fpaynet:close:func:happy:start}
              ...
              \redden{\State reduce \texttt{offChainBalance}(\alice) by $x$
              ...
              \State remove \texttt{channel} from \texttt{channels} \redden{and}
              entry from \texttt{pendingClose}(\alice)
            ...
            \bot$}
              \State assign largest \redden{$\Sigma_{\alice}$ block no. to
              entry's $h$}
            \ElsIf{there is no such closing/commitment transaction $\wedge \ h
            ...
            \EndIf
          \EndFor
          \If{\alice{} has no open channels in $\Sigma_{\alice} \redden{\wedge}
          \mathtt{negligent}(\alice) = \false \redden{\wedge}
  ...
  \begin{figure}[H]
    \begin{systembox}{\fpaynet{} -- daemon messages}
      \begin{algorithmic}[1]
        ...
        \Indent
          \redden{\State copy members of \texttt{updatesToReport}(\alice) to
          \texttt{updatesHistory}(\alice)}
          \State clear \texttt{newChannels}(\alice),
          \texttt{closedChannels}(\alice), \texttt{updatesToReport}(\alice),
          \redden{\texttt{paymentsToReport}(\alice)} and send them to \alice{}
