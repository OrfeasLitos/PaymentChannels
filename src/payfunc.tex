    ...
    \begin{systembox}{\fpaynet - pay}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \State calculate \texttt{cltvExpiry} of \texttt{charged} (as in
          Fig.~\ref{alg:protocol:pay:invoice},
          l.~\ref{alg:protocol:pay:invoice:cltv})
          \If{$\mathtt{charged} = \bot$}
            \State \Return
          \ElsIf{$\mathtt{charged} \neq \alice \vee \mathtt{charged} \notin
          \mathtt{corrupted} \ \vee$ \texttt{charged} has not been negligent
          from (the moment \texttt{charged} committed to the HTLC to the
          previous party on the $\overrightarrow{\mathtt{path}}$) +
          \texttt{delay} + $k$ until $\mathtt{cltvExpiry} - 2k - 1 \TODO{fix} \
          \vee$ (\texttt{charged} has received \textsc{poll} between
          $\mathtt{cltvExpiry} - \mathtt{relayDelay} - 2k$ \TODO{fix} and
          $\mathtt{cltvExpiry} - 2k - 1$ \TODO{fix} inclusive $\wedge$
          \texttt{charged} has received \textsc{fulfillOnChain} between the
          \textsc{poll} and $\mathtt{cltvExpiry} - k$)} \TODO{fix}
            \State halt
          \EndIf
    ...
    \begin{systembox}{\fpaynet - poll}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \mathtt{delay}\left(\alice\right) < h_{\mathrm{max}} - k$}
            \State add $\left[\mathtt{lastpoll}\left(\alice\right) + 1,
            h_{\mathrm{max}} - \texttt{delay}\left(\alice\right) - k\right]$ to
            \texttt{negligent}(\alice)
          \EndIf
          \If{$\mathtt{lastPoll}\left(\alice\right) +
          \mathtt{relayDelay}\left(\alice\right) < h_{\mathrm{max}}$}
            \State add $\left[\mathtt{lastpoll}\left(\alice\right),
            h_{\mathrm{max}} - \texttt{relayDelay}\left(\alice\right) -
            1\right]$ to \texttt{relayNegligent}(\alice)
          \EndIf
          \State $\mathtt{lastPoll}\left(\alice\right) \gets h_{\mathrm{max}}$
