\section{Payment Network Functionality}
  ...
  which \fpaynet{} takes a note to inform the corresponding player. Once more
  \fpaynet{} trusts \simulator, but later checks that the chain contains the
  correct transactions with \texttt{checkClosed}()
  ...
  The possible security violations are: channels found closed but unreported by
  \simulator{} (l.~\ref{alg:fpaynet:close:func:unnotified}), signature forgery
