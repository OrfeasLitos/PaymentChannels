\section{Perun}
  ...
  \subsection{Multistate channels}
    ...
    channel closure and of course in case of dispute over the state of a nanocontract.
    ...
    Registration happens in case of dispute with regard to the state of the nanocontract
    or when the parties want to close the multistate channel. The registration mechanism
    is very similar to that of closing a simple payment channel, so will not be described
    in detail.

    Since a nanocontract has its own $\nu\mbox{.\texttt{ver-num}}$ and
    $\nu\mbox{.\texttt{sign}}$ field, it can be updated to $\nu'$ without updating the
    whole multistate channel. The only requirement is that
    \begin{align*}
      \nu\mbox{\texttt{.blocked}}\left(Alice\right) &+
      \nu\mbox{\texttt{.blocked}}\left(Bob\right) \geq \\
      \nu'\mbox{\texttt{.blocked}}\left(Alice\right) &+
