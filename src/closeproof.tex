...
\begin{proof}
  ...
  \textit{pchid}) to \alice, in the ideal world, if it is not the first closing
  ...
  lines~\ref{alg:protocol:close:remove}
  ...
  the simulated check of line~\ref{alg:protocol:close:unilateral:ensure} in
  ...
  (\textsc{forceCloseChannel}) message to \simulator{}
  ...
  closing transaction to \ledger{} (Fig.~\ref{alg:protocol:close:unilateral},
  ...
  included (Proposition~\ref{prop:tochain}). Second, every element added to
