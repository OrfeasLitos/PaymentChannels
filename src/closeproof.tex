...
\begin{proof}
  ...
  receives \textsc{checkNew} from \environment{} (Fig.~\ref{alg:fpaynet:open},
  lines~\ref{alg:fpaynet:checkNew:included}-\ref{alg:fpaynet:checkNew:report}).
  Similarly in the real world, \alice{} populates \texttt{newChannels} either
  when she receives the second (\textsc{checkNew}) and the same funding
  transaction is in her state (Fig.~\ref{alg:protocol:checkNew},
  line~\ref{alg:protocol:checkNew:report}), or if her counterparty receives the
  second (\textsc{checkNew}), which leads to him sending \textsc{fundingLocked}
  and the funding transaction is in her state
  (Fig.~\ref{alg:protocol:fundingLocked},
  line~\ref{alg:protocol:fundingLocked}). \TODO{fix case when counterparty is
  malicious and sends a rogue \textsc{fundingLocked}} Therefore the
