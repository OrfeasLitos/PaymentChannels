...
\begin{lemma}
  ...
  \begin{gather*}
    ...
    \\
    nm \cdot \mathrm{E \mhyphen ds}(k) + 3np \cdot \mathrm{E \mhyphen ds}(k) +
    \\
...
\begin{proof}
  ...
  Fig.~\ref{alg:fpaynet:close} in case a channel is closed without using a
  ...
  Fig.~\ref{alg:fpaynet:close} in case a malicious closure by the counterparty
  was successful, in spite of the fact that \alice{} polled in time to apply the
  punishment. A (\textsc{poll}) message to \alice{} within the prescribed time
  frame (line~\ref{alg:fpaynet:close:func:ifmalicious}) would cause \fpaynet{}
  to alert \simulator{} (Fig.~\ref{alg:fpaynet:poll},
  ...
  ds}(k) + nmp \cdot \mathrm{E \mhyphen share}(k) + \mathrm{E \mhyphen prf}(k) +
