...
\begin{proof}
  ...
  (Fig.~\ref{alg:fpaynet:close}, line~\ref{alg:fpaynet:close:unilateral:send})
  for every element that it adds to \texttt{pendingClose}
  (Fig~\ref{alg:fpaynet:close}, line~\ref{alg:fpaynet:close:mark}) and this
  causes \simulator{} to submit the commitment transaction to \ledger{}
  (Fig.~\ref{alg:protocol:close:unilateral},
