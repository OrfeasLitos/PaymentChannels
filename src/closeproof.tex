...
\begin{proof}
  ...
  \redden{As far as \texttt{paymentsToReport} is concerned, in the real world it
  ...
  \redden{Because \simulator{} now sends an additional \textsc{newPayments}
  ...
  \simulator{} reports all updates with a \redden{(\textsc{newUpdate})} message
  ...
  % TODO: unredden typo
  doesn't halt, \fpaynet{} updates \redden{\texttt{offChainBalance}}(\alice) if she is
  ...
  % TODO: unredden typo
  \fpaynet{} updates \redden{\texttt{offChainBalance}}(\alice) and
  % TODO: unredden typo
  \redden{\texttt{onChainBalance}}(\alice) to reflect the event
  (Fig.~\ref{alg:fpaynet:open:negotiate},
