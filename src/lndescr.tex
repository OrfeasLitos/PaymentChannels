...
\label{sec:ov-ln}
  ...
  \noindent {\bf Two-party channels.}
    The aim of LN is to enable fast\redden{, cheap, off-chain} transactions,
    ...
    In more detail, to open a channel \alice{} and \bob{} first \redden{exchange
    ...
    counterparty. \redden{This multisig output needs signatures for both
    designated public keys in order to be spent. $F$} is funded with
    \redden{$c_F$} coins that belong only to one of the two parties, say \alice.
    ...
    transaction'' $C_A, C_B$. \redden{\alice{} uses her ``delayed payment'' key
    ...
    \begin{figure*}
      ...
      \redden{\caption{All transactions of an open channel with an HTLC in
    ...
    She now broadcasts $F$; once \redden{both parties see that} it is confirmed,
    \redden{they generate and exchange new ``commitment'' keys (used for
    ...
    funding output \redden{on-chain; doing this for every update would however
    ...
    benefit in doing so. \redden{The same reasoning holds if \bob{} broadcasts
    ...
    \redden{$sk_{\mathrm{dcom}, A}$} which is a usual ECDSA key, or with a
    signature by \bob's ``revocation'' secret key \redden{$sk_{\mathrm{rev},
    ...
    by combining one secret value \redden{$sk_{\mathrm{com}, n, A}$} that
    \alice{} knows and one secret value \redden{$sk_{\mathrm{com}, n, B}$} that
    \bob{} knows. \alice{} therefore can prevent \bob{} from using his
    revocation key until she sends \redden{him $sk_{\mathrm{com}, n, A}$}.
    Therefore, \alice{} will send \redden{\bob{} $sk_{\mathrm{com}, n, A}$} only
    ...
    \redden{already mentioned} ``relative timelock''. If \alice{} broadcasts a
    ...
    \redden{negotiated during the opening of the channel and} hardcoded in the
    ...
    \redden{and any outstanding HTLC transactions (explained below)} and wait
    ...
    broadcasting the ``closing transaction'' \redden{which is not encumbered
    ...
    \redden{Furthermore,} LN makes it possible to trustlessly outsource this to
  ...
  \noindent {\bf Multi-hop payments.}
    ...
    trust is needed. \redden{It is realised as an additional output of the
    ...
    \bob{} and \charlie, two HTLC \redden{transactions} were signed and
    fulfilled \redden{in total} for the payment to go through. Two updates
    ...
    \redden{Figure~\ref{fig:all-txs} shows all transactions implicated in a
    ...
    \redden{for the benefit of avoiding the added complexity and making it
