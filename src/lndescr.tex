...
\label{sec:ov-ln}
  ...
  \noindent {\bf Two-party channels.}
    ...
    In more detail, to open a channel \alice{} and \bob{} first exchange their
    keys and desired timelocks (explained below) and then build locally
    ...
    broadcasts $F$; once both parties see that it is confirmed, they generate
    and exchange new ``commitment'' keys (used for updating the channel later)
    ...
    The last element needed to make channel updates secure is the so called
  ...
  \noindent {\bf Multi-hop payments.}
    ...
    two HTLC transactions were signed and fulfilled for the payment to go
    through. Two updates happened in each channel: one to sign the HTLC and one
    to fulfill it. The time frames were chosen so that every intermediary has
    had the time to learn the preimage and give it to the previous party on the
    path.
