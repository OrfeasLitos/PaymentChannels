...
\label{sec:ov-paynet}
  ...
  payment network. \redden{Its definition can be found in the full
  version~\cite{full-version}}. The central aim of \fpaynet{} is opening payment
  ...
  \begin{itemize}
    ...
    \begin{enumerate}
      ...
      - \tochain$ with a \textsc{fulfillOnChain} message, as discussed below.
    \end{enumerate}
    ...
    halts. In case the charged party was honest and non-negligent, \fpaynet{}
    halts. It also halts if a particular payment resulted in a channel update
    for which \simulator{} did not inform \fpaynet{}.
    \item \fpaynet{} executes the function
    \texttt{checkClosed}($\Sigma_{\alice}$) every time it receives
    $\Sigma_{\alice}$ from \ledger{}.  In this case, it checks that every
