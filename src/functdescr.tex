...
\label{sec:ov-paynet}
  ...
  \begin{itemize}
    ...
    multi-hop payments that include the identity of the party that is charged. Moreover, 
    for each resolved payment, the message includes two
    ...
    (a) the one that initiated the payment, (b) a
    malicious party or (c) an honest party that is negligent. The latter case
    ...
    \begin{enumerate}
      ...
      \item did not enforce the retrieval of the funds lost as a result of this payment 
      when the chain in her view
      had height $\mathtt{IncomingCltvExpiry} - \tochain$
      (Fig.~\ref{alg:fpaynet:resolvepay},
