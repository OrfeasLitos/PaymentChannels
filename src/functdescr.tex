...
\label{sec:ov-paynet}
  ...
  \textit{tid}) from \alice, it informs \redden{simulator} \simulator{} of the
  intention of \redden{environment} \environment{} to create a channel between
  ...
  $\overrightarrow{\mathtt{path}}$. \redden{In the same vein}, when \fpaynet{}
  receives (\textsc{closeChannel}, \texttt{receipt}, \textit{pchid}) or
  % TODO: unredden typo
  (\textsc{forceCloseChannel}, \texttt{receipt}, \textit{pchid}) from
  \redden{\alice{}} (for a cooperative or unilateral close respectively), it
  ...
  \begin{itemize}
    ...
    \alice's latest \redden{state} $\Sigma_{\alice}$ and verifies that no bad
    ...
    Refer to lines~\ref{alg:fpaynet:poll:ifdsforgery}
    ...
    values\redden{, expressed in absolute} block height:
    \texttt{OutgoingCltvExpiry}, which is the \redden{highest} block in which
    the charged party could claim money from the previous hop (closer to the
    \redden{payer}) and \texttt{IncomingCltvExpiry}, which is the
    \redden{lowest} block in which the charged party could claim money from the
    next hop (closer to the \redden{payee}). \fpaynet{} checks that for each
    ...
    \begin{enumerate}
      ...
      \item did not enforce the retrieval of the funds lost as a result of this
      payment when the chain in her view had height $\mathtt{IncomingCltvExpiry}
      - \tochain$ (Fig.~\ref{alg:fpaynet:resolvepay:haltcond:abs}) with a
    ...
    ensures that the two expiries (\texttt{OutgoingCltvExpiry} and
    \texttt{IncomingCltvExpiry}) have a distance of at least
    $\mathtt{relayDelay}(\alice) + \tochain$, otherwise it halts. In case the
