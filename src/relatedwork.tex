...
\label{appendix:relwork}
  ...
  Furthermore, any realistic model of the network should consider the fact that
  messages reach their destination with a delay and possibly in a different
  order from the one they were sent. Therefore it is plausible to believe that
  such a ledger functionality is not realizable on top of realistic network
  models, even when corruptions are not allowed. We therefore conclude that such
  a ledger is ``too ideal.'' \TODO{rephrase to fit Perfect Ledger
  Appendix~\ref{appendix:perfectledger}}
