...
assumptions and complicated the setup needed \orfeas{check prev}. The additional
...
approach where it is shown that if the assumption is raised to a security
threshold of $3/4$ plus the honesty of an additional special player it is
possible to obtain optimal latency. Nevertheless, the lightning network is
currently the only option that readily interoperates with the Bitcoin
blockchain.

Despite the importance of the lightning network for low latency payments over
Bitcoin there is still no work so far providing a thorough formal security
...
security properties that are provided by the lightning network. \orfeas{The
approach taken can be applied to any payment network by just changing the exact
...
\item \orfeas{check} We prove that a ``perfect'' ledger functionality, i.e. a
...
\orfeas{we had this here: The main advantage of
...
strong network assumptions (c.f. Appendix~\ref{appendix:perfectledger}).}
...
Section~\ref{appendix:perfectledger}. \orfeas{check prev} Then we give more
