...
\url{https://raiden.network}.} that \redden{functions over the Bitcoin
...
connects the two end-points of the transaction. \redden{Implicated parties are
guaranteed that, if they wish so,} \emph{eventually} the ledger will record
\redden{a} ``gross'' settlement \redden{transaction that reflects the balance
resulting from all in-channel payments}. Deviations from this guarantee are
cryptographically feasible but deincentivised\redden{: a malicious party trying
...
security analysis of the lightning network\redden{, which we carry out} in the
...
security properties provided by the lightning network. \redden{We describe our
...
tuning the \redden{following parts of the functionality:} the exact channel
...
are in line with the off-chain \redden{ones}. In order to handle
\redden{realistic} adversarial deviations, \redden{$\fpaynet{}$ allows} the
adversary to \redden{choose one of the following outcomes for each multi-hop
...
the path. This last outcome is a crucial security characteristic \redden{of the}
lightning network: honest parties are \redden{required} to poll the
functionality \redden{with a frequency} that corresponds to their level of
involvement in the network and the properties of the underlying ledger. If a
party \redden{does not poll often enough, $\fpaynet{}$ identifies it as
negligent} and it may lose funds.

\item We identify for the first time the exact polling requirements \redden{so
that honest parties} do not lose funds\redden{, as a function of the parameters
of $\ledger$.} The polling requirements for each party are two-fold: (i) the
first type refers to monitoring for closures of channels \redden{of which the
party is a member}, and is specified by the parameter ``$\mathrm{delay}$''
\redden{(chosen by the party)}, (ii) the second type refers to monitoring for
specific events related to receiving and relaying payments\redden{. In detail,
let \emph{Alice} be an intermediary of a multi-hop payment. When the payment
starts, she specifies two blockheights $h, h'$. Also, let $a$} the upper bound
...
included in a block in the ``stable'' part of the ledger). \redden{\emph{Alice}
...
separated by a time window \redden{so that the chain can} grow by at least $a$
blocks.

\item We provide a complete pseudocode description of the lightning network
protocol $\Pi_{\mathrm{LN}}$ and prove that it realises $\fpaynet{}$ \redden{in
...
\redden{most} of these primitives are quite standard (a PRF, a Digital Signature
scheme and an Identity Based Signature scheme)\redden{, the remaining one} is
slightly less standard. \redden{The \emph{combined digital signature} -- as we
chose to call it --} is a special case of an asymmetric two-party digital
signature primitive (e.g., see~\cite{DBLP:conf/ndss/NicolosiKDM03} and
references therein) with the \redden{following} characteristic\redden{:} One of
the two parties, the shareholder, generates and stores a share of the signing
key\redden{.} The public key of the combined signature \redden{can be}
determined non-interactively \redden{from public} information produced by both
...
construction lying within the \redden{lightning} specification \redden{realises}
...
to formalize payment networks, \redden{as well as highlights the considerable
latency improvement that the lightning protocol offers in comparison to directly
using the ledger.}
\end{enumerate}
...
such as Ethereum~\cite{perun}, hardwar\redden{e} assumptions, see
...
presented ideal functionalities abstracting the concept\redden{, but they did
...
assumptions (cf. Section~\ref{sec:perfect-ledger}). \redden{It is worth noting
...
way, but a complete formalization of the ledger is missing. \redden{Lastly, the
...
proof \redden{of the main simulation theorem}. Finally, in
