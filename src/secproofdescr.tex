...
\label{sec:ov-security-proof}
  ...
  \begin{proofsketch}
    The proof is done in 5 steps \redden{of successive games replacement}. In
  ...
  \end{proofsketch}
  \redden{As a concrete example of the proof approach, proving
  Lemma~\ref{lemma:reg} entails the following parts: First
  ...
  (Fig.~\ref{alg:sim:reg}), which simulates all protocol instances, but in
  ...
  functionality and simulator that were used in Lemma~\ref{lemma:dummy} can be
