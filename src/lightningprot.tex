\ \\ \noindent {$\Pi_{\mathrm{LN}}^t$} The protocol is executed by player $Alice$.
  ...
  \begin{algorithmic}[1]
    ...
    \Procedure{Open}{source, dest, funds}
      ...
      \If{response == yes}
        \State $\left(\left(pa_0, sa_0\right), \left(pa_1, sa_1\right), \left(pa_2,
        ...
        \mathcal{F}_{\mathrm{Wallet}}.\mathrm{isValidPubKey}\left(pb_i\right)$ == true}
          \State $\left(F, C1b, RD1b\right) \gets \textsc{GetOpenTXs}\left(pa_0, pa_1,
          pb_1, pa_2, pb_2, pb_3, \mathrm{funds}\right)$
          \State Send (openTXs, $\left(F, C1b, RD1b\right)$) to dest
          \State Wait for response
          %TODO start
          \If{response == $\left(C1a, RD1a\right) \And C1a, RD1a$ correct commitment,
          revocable delivery tx}
            \State Send signature to $F_0$
    ...
    \Procedure{GetOpenKeys}{$x$}
      \State $\left(pa0, sa0\right) \leftarrow
      \mathcal{F}_{\mathrm{Wallet}}.\mathrm{getKeyWith}\left(x\right)$ \Comment{Contains
      exactly $x$ coins}
