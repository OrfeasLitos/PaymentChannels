\ \\ \noindent {$\Pi_{\mathrm{LN}}$} The protocol is executed by player $Alice$.
  ...
  \begin{algorithmic}[1]
    ...
    \EndIndent
    \State
    \Function{OpenChannel}{source, fundee, initTX, forFundTX, coins, delay, bcast}
      ...
      fundee
      \State assert(response = (\texttt{fundingSigned}, id, sig) $\And \mathrm{id} =
      \mathcal{H}\left(F\right) \oplus \mathrm{idx}$
      \Indent
    ...
    \Function{GetOpenKeys}{}
      ...
      \mathcal{F}_{\mathrm{Wallet}}.\mathrm{genKey}\left(\right)$ \Comment{For what? TODO}
    ...
    \EndFunction
    \State
    \State Upon receiving (\texttt{openChannel}, coins, delay, bcast,
    \Indent
      \Indent
        \State $pa_F, pa_{\mathrm{pay}}, pa_{\mathrm{dpay}}, pa_{\mathrm{htlc}},
    ...
    \EndIndent
    \Indent
      \State Send (\texttt{recvOpen}, $\mathrm{pid}_{Bob}$, coins, delay, bcast,
      \Indent
        \Indent
          \State $pa_F, pa_{\mathrm{pay}}, pa_{\mathrm{dpay}}, pa_{\mathrm{htlc}},
      ...
      \EndIndent
      \State $\left(\mathrm{txid}, \mathrm{id}, \mathrm{success}\right) \gets
      \textsc{AcceptChannel}\left(TODO\mathrm{pid}_{Alice}, \mathrm{pid}_{Bob},\right.$
      \Indent
        \Indent
          \State $\left.\mathrm{initTX}, \mathrm{forFundTX}, \mathrm{coins},
          \mathrm{delay}, \mathrm{bcast}\right)$
    ...
    images?
    \State TODO: Bob's side, P2WPKH(), msig(), sigFor\_()
    \State
