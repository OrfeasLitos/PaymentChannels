\ \\ \noindent {$\Pi_{\mathrm{LN}}$} (self is \alice)
  ...
  \begin{algorithmic}[1]
    ...
    \State \orfeas{PROBLEM: 1 input many outputs}
    ...
    \State \orfeas{@Aggelos: maybe leak queries to global functionalities to
    \adversary?}
    ...
    \Indent
      ...
      \If{$\delta = \mathtt{receiver}$}
        ...
        \mathtt{absDelay}'$
        \State \orfeas{TODO: fulfill HTLC}
      \Else \Comment{We are an intermediary}
        ...
        \State increment $\mathtt{HTLCNo}_{\textit{pchid}}$ by one
        \State \orfeas{TODO: build remoteCom (find underscript), HTLC-(what?)
        (find both underscripts)}
        ...
        \State TODO: generate new keys \orfeas{may have to move out of If}
        ...
        n+1},\right.\right.$
        $\left.\left.\mathtt{cltv\_expiry} \text{ absolute}\right) \vee
        pt_{\mathrm{htlc}, n+1}, \text{on preimage of } h\right)$\}
        \For{i from 1 to $m+1$}
          \State $\mathtt{HTLCTimeout}_{n+1, i} \gets$ TX \{input: output
          $\mathrm{htlc}_{n+1, i}$ of $\mathtt{remoteCom}_n$, output:
          $\left(x, \left(pt_{\mathrm{rev}, n+1} \vee ph_{\mathrm{dpay}, n+1},
          \mathtt{delay} \text{ relative}\right)\right)$\} \orfeas{may have to
          sign HTLCsuccess...}
        \EndFor
        ...
        \For{i from 1 to $m+1$}
          \State add $\mathtt{signature}\left(\mathtt{HTLCTimeout}_{n+1, i},
    ...
    \State Upon receiving (\textsc{commitmentSigned}, \orfeas{TODO}) from \bob:
    \Indent
      \State send (\textsc{revokeAndAck}, \orfeas{TODO}) to \bob
    \EndIndent
    \State

    \State Upon receiving (\textsc{revokeAndAck}, \orfeas{TODO}) from \bob:
    \Indent
      \If{terminal node}
    ...
    \State Upon writing useless crap:
      \If{no hops}
