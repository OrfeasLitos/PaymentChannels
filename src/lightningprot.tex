\ \\ \noindent {$\Pi_{\mathrm{LN}}$} (self is \alice)
  ...
  \begin{algorithmic}[1]
    ...
    \environment: \Comment{new msg: represents lnd polling daemon}
    \Indent
      \State ensure there is a matching \texttt{channel} in
      \texttt{pendingOpen}, marked as ``broadcast''
      \State send (\textsc{read}) to \ledger{} and assign reply to
      $\Sigma_{\alice}$
      \State esure there is an unspent TX in $\Sigma_{\alice}$ with ID
      ...
      \State mark \texttt{channel} in \texttt{pendingOpen} as ``waiting for
      \textsc{fundingAck}''
    ...
    \State Upon receiving (\textsc{fundingAck}, \textit{pchid},
    $pt\_\mathrm{base}_{\mathrm{com}, 2}$) from \bob:
    \Indent
      \State ensure there is a \texttt{channel} in \texttt{pendingOpen} marked
      as ``waiting for \textsc{fundingAck}''
      \State move channel data from \texttt{pendingOpen} to \texttt{channels}
    ...
    \Indent
      ...
      outputs:
      \State $\left(c_{\alice} - x, pt_{\mathrm{rev}, n+1} \vee
      \left(ph_{\mathrm{dpay}, n+1}, \mathtt{delay} \text{
      relative}\right)\right), \left(c_{\bob}, pt_{\mathrm{pay},
      n+1}\right),$ $\left(c_{n, i}, \text{htlc methods}_{n+1,
      i}\right)_{\left[1, \dots, m\right]}, \left(x, pt_{\mathrm{rev}, n+1}
      \vee \left(pt_{\mathrm{htlc}, n+1} \wedge ph_{\mathrm{htlc},
      n+1},\text{on}\right.\right.$ $\left.\left.\text{preimage of } h\right)
      \vee pt_{\mathrm{htlc}, n+1}, \mathtt{cltv\_expiry} \text{
      absolute}\right)$\}
