\ \\ \noindent {$\Pi_{\mathrm{LN}}$} (self is \alice)
  ...
  \begin{algorithmic}[1]
    ...
    \Indent
      ...
      \text{ relative}\right)$\} \Comment{HTLC success}
    ...
    \Indent
      ...
      \State generate local and remote keys
      \State generate local commitment tx
      \State ensure $\mathtt{Verify}\left(\mathtt{localCom}_{n+1},
      \mathtt{comSig}_{n+1}, pt_F\right) = \mathtt{true}$
      \For{$i$ from $\mathtt{lastComHTLC}_{\mathit{pchid}}$ to
      $\mathtt{HTLCNo}_{\mathit{pchid}}$}
        \State generate local HTLCTimeout/HTLCSuccess
        \State ensure $\mathtt{Verify}\left(\mathtt{HTLCTimeout/Success}_{n+1,
        i\orfeas{TODO: fix i}}, \mathtt{HTLCSigs}_{n+1} \left[i\right],
        pt_{\mathrm{htlc}, n+1}\right) =
        \mathtt{true}$
      \EndFor
      ...
      $\mathtt{HTLCNo}_{\mathit{pchid}}$}
        ...
        \State generate
        $\mathtt{signature}\left(\mathtt{HTLCTimeout/Success}_{n+1,
        i\orfeas{TODO: fix i}}, sh_{\mathrm{htlc}, n+1}\right)$
        \State add to HTLCSigs
      \EndFor
      ...
      ph_{\mathrm{com}, n+1}$) to \bob
    \EndIndent
    \State

    \State Upon receiving (\textsc{revokeAndAck}, \textit{pchid},
    $sh_{\mathrm{com}, n}, ph_{\mathrm{com}, n+1}$) from \bob:
    \Indent
