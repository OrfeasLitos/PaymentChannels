  ...
  \begin{algorithmic}[1]
    ...
    $\mathcal{E}$:
      ...
      x$}
        ...
        \State tx $\gets$ \textsc{Open}($\mathrm{sid}_{Alice}$, $\mathrm{sid}_{Bob}$, $x$)
      ...
      \EndIf
    \State
    \Procedure{Open}{source, dest, funds}
      \State Send (proposeChannel, funds) to dest
      \State Wait for response
      \If{response == yes}
        \State $\left(\left(pa_1, sa_1\right), \left(pa_2,
        sa_2\right), \left(pa_3, sa_3\right)\right) \gets
        \textsc{GetOpenKeys}\left(\mathrm{funds}\right)$
        \State Send (openKeys, $\left(pa_0, pa_1, pa_2, pa_3\right)$) to dest
        \State Wait for response
        \If{response == $\left(pb_1, pb_2, pb_3\right) \And \forall i \in \left[3\right],
        \mathcal{F}_{\mathrm{Wallet}}.\mathrm{isValidPubKey}\left(pb_i\right)$ == true}
          ...
          \left(\mathrm{funds}\right)$ \Comment{Returns tx with 1 p2pkh output of value
          funds and the keypair that spends it}
          \State $\left(F, C1b, RD1b\right) \gets
          \textsc{GetOpenTXsSend}\left(\mathrm{initTX}, pa_0, pa_1, sa_1, pb_1, pa_2,
          sa_2, pb_2, pb_3, \mathrm{funds}\right)$
          \State Send (openTXs, $\left(F, C1b, RD1b\right)$) to dest
          \State Wait for response
          \If{response == $\left(C1a, RD1a\right) \And
          %TODO start
    ...
    \EndProcedure
    \State
    \Procedure{GetOpenKeys}{$x$}
      \State $\left(pa1, sa1\right) \leftarrow
      \mathcal{F}_{\mathrm{Wallet}}.\mathrm{genKey}\left(\right)$ \Comment{For funding tx
      output}
      \State $\left(pa2, sa2\right) \leftarrow
      \mathcal{F}_{\mathrm{Wallet}}.\mathrm{genKey}\left(\right)$ \Comment{For C1a
      outputs}
      \State $\left(pa3, sa3\right) \leftarrow
      \mathcal{F}_{\mathrm{Wallet}}.\mathrm{genKey}\left(\right)$ \Comment{For RD1a
      output}
      \State Return $\left(\left(pa0, sa0\right), \left(pa1, sa1\right), \left(pa2,
      sa2\right), \left(pa3, sa3\right)\right)$
    \EndProcedure
    \State
    \Procedure{GetOpenTXsSend}{$pa_0, pa_1, sa_1, pb_1, pa_2, sa_2, pb_2, pb_3,
    \mathrm{funds}$}
      \State $F \gets \mathcal{F}_{\mathrm{Wallet}}$.getTX(\{
      \State inputs: $\left[\left(\mathrm{P2PKH}\left(pa_0, \mathrm{funds}\right),
      \bot\right)\right]$
    ...
    \Procedure{GetOpenTXsRespond}{$F, sb_1, pa_2, pb_2, sb_2, pa_3, \mathrm{funds}$}
      ...
      \State inputs: $\left[\left(\mathcal{F}_{\mathrm{Wallet}}.\mathrm{outpoint}\left(F,
      0\right),
      \text{sigFor2-of-2-msig}\left(\mathcal{F}_{\mathrm{Wallet}}.\mathrm{outpoint}\left(F,
      0\right), 1, sb_1\right)\right)\right]$
      ...
      \State inputs:
      $\left[\left(\mathcal{F}_{\mathrm{Wallet}}.\mathrm{outpoint}\left(C1a, 0\right),
      \text{sigForP2PKH}\left(\mathcal{F}_{\mathrm{Wallet}}.\mathrm{outpoint}\left(C1b,
      0\right), sb_2\right)\right)\right]$
