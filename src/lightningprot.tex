\ \\ \noindent {$\Pi_{\mathrm{LN}}^t$} The protocol is executed by player $Alice$.
  ...
  \begin{algorithmic}[1]
    ...
    \Procedure{OpenChannel}{source, dest, funds}
      ...
      \EndIndent
      \State Send (\texttt{fundingSig}, $\mathcal{F}_{\mathrm{Wallet}}.\text{sigForP2PKH}
      \left(\mathrm{outpoint}\left(\mathrm{initTX}, 0\right), sa_0\right)$
      \State Wait for $F$ to be confirmed on-chain
      \State assert($F$ confirmed, dest did not broadcast) \Comment{dest broadcasts $F$
    ...
    \EndProcedure
    \State
    \Procedure{isValidOpenTXsSend}{$\left(F, C1b, RD1b\right), pa_1, pb_1, pa_2, pb_2,
    pb_3, \mathrm{funds}$}
      ...
      \State assert($C1b_0 = \mathrm{P2PKH}\left(pa_2, 0\right)$)
      \State assert($C1b_1 = \mathrm{RSMC}\left(pa_2, pb_2, t, 0\right)$) ????
      \State assert($RD1b$ has exactly 1 output)
      \State assert($RD1b_0 = \mathrm{Delay}\left(pb_3, t, \mathrm{funds}\right)$) ????
      \State Return True
    \EndProcedure
    \State
    \Procedure{isValidOpenTXsResponse}{$\left(C1a, RD1a\right), pb_2, pa_2, pa_3,
    \mathrm{funds}$}
      \State assert($C1a$ has exactly 2 outputs)
      \State assert($C1a_0 = \mathrm{P2PKH}\left(pb_2, 0\right)$)
    ...
    think yes)
    \State TODO: isValidOpenTXsSend(), Bob's side, P2PKH(), msig(), RSMC(), Delay(),
    sigFor\_(), use actual spec
...
\ \\ \noindent {Connection with $\mathcal{F}_{\mathrm{Ledger}}$}
  ...
  \begin{algorithmic}[1]
    ...
    \State
    \State Send (\texttt{READ}, sid) to $\mathcal{F}_{\mathrm{Ledger}}$ \Comment{Whose
    sid? List is not created by Alice}
