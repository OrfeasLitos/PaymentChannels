\ \\ \noindent {$\Pi_{\mathrm{LN}}$} The protocol is executed by player $Alice$.
  ...
  \begin{algorithmic}[1]
    ...
    \Indent
      ...
      in \texttt{pending}
      \State $\mathtt{blocked} \leftarrow \mathtt{blocked} + x$
    ...
    \State TODO: make \texttt{channels} look more like \texttt{channels} of
    ...
    $\overrightarrow{\mathtt{path}}$, \texttt{receipt}) from $\mathcal{E}$:
      ...
      \ElsIf{hops}
        ...
        \If{$\mathcal{H}\left(\mathrm{preimage}\right) = \mathrm{hash}$}
          ...
          \State $\mathtt{channels} \leftarrow \mathtt{channels} \setminus
          \left\{e\right\}$
          \State $\mathtt{channels} \leftarrow \mathtt{channels} \cup
