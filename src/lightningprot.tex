\ \\ \noindent {$\Pi_{\mathrm{LN}}$} The protocol is executed by player $Alice$.
  ...
  \begin{algorithmic}[1]
    ...
    \mathrm{pid}_{Bob}, \mathrm{forFundTX}, \mathrm{coins}, \mathrm{delay},
    \mathrm{bcast}\right)$ from $\mathcal{E}$:
    \Indent
    ...
    forFundTX, w/ Alice}
      ...
      \State \texttt{witnessScript} $\gets$ "2 " + $pa_F$ + " " + $pb_F$ + " 2
      \texttt{OP\_CHECKMULTISIG}"
      \State \texttt{hash} $\gets$ SHA256(\texttt{witnessScript})
      \State $\left(F, \mathrm{idx}\right) \gets \mathtt{tx}$.addedOutput(
      \{scriptPubKey: "0 " + \texttt{hash}, value: coins\}) \Comment{TODO:
      continue from here}
