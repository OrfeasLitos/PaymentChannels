...
\label{sec:preliminaries}
  ...
  \noindent \textbf{Universal Composability framework.}
    ...
    \redden{\environment{}} cannot distinguish between the real world and the
    ...
    facilitate this\redden{,} the Universal Composability~\cite{uc} framework
    \redden{allows us} to analyse a single instance of the protocol and then
    ...
    \redden{As mentioned, lightning network members have to periodically check
...
%    to leverage previous results on the security of blockchains.
  ...
  \noindent \textbf{Hybrid functionalities used.}
    ...
    valid, will be added to the ledger \redden{and made visible to all parties}
    ...
    in Appendix~\ref{appendix:ledger}. \redden{The current work makes heavy use
    ...
    Furthermore, \ledger{} needs \redden{the} \Fclock \redden{functionality,
    ...
    \redden{As already mentioned,} the protocol and functionality defined in the
    current work do not make direct use of \Fclock. \redden{Indeed, the only
    ...
    Bitcoin node and \ledger.} We therefore omit \redden{it} in the statements
    of Lemmas~\ref{lemma:dummy}-\ref{lemma:close} and
    Theorem~\ref{theorem:simulation} for simplicity of notation; \redden{it}
    should normally appear as hybrid along with \ledger. \redden{Its} exact
    definition can be found in Appendix~\ref{appendix:ledger}. We also note that
    \ledger{} and \Fclock are global functionalities~\cite{globaluc} \redden{and
  ...
  \noindent \textbf{Cryptographic Primitives.}
    ...
    functions, see \redden{Appendices}~\ref{sec:ds},~\ref{sec:ibs},
    \redden{and}~\ref{sec:prf}. Finally, a less common  two-party cryptographic
