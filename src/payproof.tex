  ...
  \begin{simulatorbox}{$\simulator{}_{\mathrm{LN - Reg - Open - Pay}}$}
    ...
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        \State \TODO{an honest relay-negligent player (i.e. that hasn't received
        poll) pays if the counterparty goes on-chain and spends the commitment
        tx}
        \State \TODO{a malicious player pays in the previous case and if it does
        not go on-chain or send uFH back and the counterparty goes on-chain and
        spends the commitment tx}
