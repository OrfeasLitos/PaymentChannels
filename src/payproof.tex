\begin{figure}[!htbp]
  \begin{systembox}{$\fpaynet{}_{\mathrm{, Pay}}$}
    \begin{algorithmic}[1]
      ...
      \redden{\textsc{adversarySendInvoice}}, \textsc{pushAdd},
...
\begin{figure}[!htbp]
  \begin{simulatorbox}{$\simulator{}_{\mathrm{LN - Reg - Open - Pay}}$ -- pay}
    ...
    \begin{algorithmic}[1]
      \State \redden{Upon simulating line~\ref{alg:protocol:pay:invoice:receive}
      of Fig.~\ref{alg:protocol:pay:invoice}:}
      \Indent
        \redden{\If{$\exists (\bot, \bot, \mathit{payid}) \in \mathtt{payids}$
      ...
      \Indent
        \State add ($\overrightarrow{\mathtt{path}}$, \redden{$\top$,}
        \textit{payid}) to \texttt{payids}
        \State strip \textit{payid} \redden{and \alice}, simulate receiving the
        ...
        \textsc{updateAddHtlc}} \Comment{payment failed}
          ...
          \State remove ($\overrightarrow{\mathtt{path}}$, \redden{\_,}
...
\begin{figure}[!htbp]
  ...
  \redden{adversarial payment}}
    \begin{algorithmic}[1]
      \redden{\State Upon receiving (\textsc{sendInvoice}, $x$,
      ...
      \redden{\State Upon receiving (\textsc{adversarySendInvoice},
  ...
  \begin{simulatorbox}{$\simulator{}_{\mathrm{LN - Reg - Open - Pay}}$ -- push}
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        ITI}
          \State send (\redden{\textsc{newUpdate}}, \texttt{receipt}, \alice) to
          $\fpaynet{}_{\mathrm{, Pay}}$, where \texttt{receipt} is the
          \redden{field} just added to the simulated \texttt{updatesToReport}
      ...
      \textsc{pushFulfill}, \textsc{pushAdd}, \textsc{commit}}
        ...
        \label{alg:sim:resolvepay:mayberesolve}
          ...
          \label{alg:sim:resolvepay:everystate}
            \ForAll{$(\overrightarrow{\mathtt{path}}, \redden{\_,}
          ...
          \State \redden{clear \texttt{charged} and send
  ...
  timely fulfill}
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        \redden{anymore} and a tx that spends the delayed output of
        ...
        \alice{} and the \texttt{HTLC-\redden{timeout}} of the relevant HTLC
      ...
      \Statex \textit{Effect}:
      \Indent
        \State add (\alice, \textit{payid}) to \texttt{charged}(\redden{payer})
        \redden{\If{there is an $h \neq \top$ member in \texttt{payids} entry}
        \Comment{adversarially initiated payment, functionality doesn't know
        path}
          \State append $\overrightarrow{\mathtt{path}}$ to
  ...
  payment}
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        \textsc{commit} and \textsc{revokeAndAck}) OR \redden{(}player after
        \alice{} has irrevocably fulfilled the HTLC on-chain (i.e. his
        \texttt{HTLC-success} is in $\Sigma_{\alice}$) \redden{AND \alice{} has
...
\begin{proof}
  Before focusing on individual messages sent by \environment{} \redden{or
  ...
  \redden{In the ideal world, when the simulated \adversary{} attempts to send a
  ...
  \redden{In the ideal world, when the simulated \adversary{} attempts to add an
  ...
  \overrightarrow{\mathtt{path}}, \redden{\mathit{expayid}}\right)$ to \alice{}
  ...
  \redden{\_}\right)$ and \bob{} is corrupted), or if the handling of the
  ...
  line\redden{s}~\ref{alg:fpaynet:resolvepay:loops:halt:do}\redden{,}~\ref{alg:fpaynet:resolvepay:loops:indiffs:debit:halt}
  \redden{and}~\ref{alg:fpaynet:resolvepay:loops:indiffs:credit:halt}), it
  ...
  \redden{We will now show that the halt of
  ...
  \redden{Moving on, we have to prove} that the halt of
  ...
  \textsc{revokeAndAck} message, it logs the \redden{new receipt} in
  ...
  \redden{It remains to be proven that \fpaynet{} does not halt with
  ...
  \redden{We will now prove that the halt of
