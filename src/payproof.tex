...
\begin{figure}[!htbp]
  \begin{simulatorbox}{$\simulator{}_{\mathrm{LN - Reg - Open - Pay}}$ -- pay}
    ...
    \begin{algorithmic}[1]
      ...
      from $\fpaynet{}_{\mathrm{, Pay}}$:
      \Indent % TODO: CONTINUE with doing something with expayid
...
\begin{figure}[H]
  ...
  corrupted charged}
    \begin{algorithmic}[1]
      ...
      \Statex \textit{Effect}:
      \Indent
         \State add to \texttt{charged}(\alice) a tuple (\texttt{corrupted},
         \textit{payid}) where \texttt{corrupted} is set to one of the corrupted
         parties between \alice{} and \bob{}
         \State remove ($\overrightarrow{\mathtt{path}}$, \textit{payid}) from
         \texttt{payids}
      \EndIndent
    \end{algorithmic}
