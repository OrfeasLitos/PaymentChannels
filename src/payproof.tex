...
\begin{proof}
  ...
  \neg R$ holds.
  % TODO: specify exact bounds for P, Q, R
  ...
  $E_2$). Observe that $E = E_1 \wedge E_2$. We note that the elements in
  ...
  $E_{1,1}: \mathtt{tx}_2$ spends the delayed output with a signature valid by
  the delayed payment public key after the locktime expires. $E_{1,2}:
  \mathtt{tx}_2$ spends the delayed output with a signature valid by the
  revocation public key $p_{\alice, \mathrm{rev}}$. Note that $E_1 = E_{1,1}
  \vee E_{1,2}$ and $E_{1,1}, E_{1,2}$ are mutually exclusive (since the same
  output cannot be spent twice). Observe that $E_{1,2} \subset R$, thus
  $\Pr\left[E_{1,2} | \neg R\right] = 0$. We now concetrate on the event
  $E_{1,1}$. Due to the fact that $\mathtt{tx}_2$ spends an output locked with a
  relative timelock of length $\mathtt{delay}\left(\alice\right) + \tochain$,
  ...
  \Pr\left[E_{1,2}\right] = 0$. We conclude that, given $\neg P \wedge \neg Q
  \wedge \neg R$ \textsc{poll} introduces no opportunity for
  distinghuishability.
  ...
  \dave{} has \textsc{poll}ed in time twice to learn the preimage of the HTLC
  early enough (event $C$) and has attempted to fulfill on chain at the right
  moment (event $D$)) (Fig.~\ref{alg:fpaynet:resolvepay},
