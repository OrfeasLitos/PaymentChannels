...
\begin{figure}[!htbp]
  \begin{simulatorbox}{$\simulator{}_{\mathrm{LN - Reg - Open - Pay}}$ -- pay}
    ...
    \begin{algorithmic}[1]
      ...
      \Indent
        \If{$\exists (\bot, \bot, \mathit{payid}) \in \mathtt{payids}$ where
        \textit{payid} is the one in the \textsc{invoice} message}
      ...
      \Indent
        ...
        \If{during the simulation above, line~\ref{alg:protocol:pay:foc:payment}
        is simulated in \alice's ITI}
      ...
      \Indent
        ...
        \If{during the simulation above, line~\ref{alg:protocol:poll:report:pay}
        of Fig.~\ref{alg:protocol:poll:closedch} is simulated in \alice's ITI}
          \State send (\textsc{newPayments}, \texttt{payment}, \alice) to
          $\fpaynet{}_{\mathrm{, Pay}}$, where \texttt{payment} consists of the
          tuple just added to the simulated \texttt{paymentsToReport}, appended
      ...
      \textsc{pushFulfill}, \textsc{pushAdd}, \textsc{commit}}
        ...
        \label{alg:sim:resolvepay:mayberesolve}
          ...
          \label{alg:sim:resolvepay:everystate}
            \ForAll{$(\overrightarrow{\mathtt{path}}, \_, \mathit{payid}) \in
