...
\begin{proof}
  ...
  of each affected channel (line~\ref{alg:fpaynet:resolvepay:update}) and does
  not send any message, thus control returns to \environment. Therefore we have
  to prove that $\fpaynet{}_{\mathrm{, Pay}}$ halts with at most negligible
  ...
  line~\ref{alg:fpaynet:resolvepay:update:halt} in
  ...
  not been declared or confirmed as closed (Fig.~\ref{alg:fpaynet:resolvepay},
  lines~\ref{alg:fpaynet:resolvepay:updateloop}
  and~\ref{alg:fpaynet:resolvepay:haltloop}), all channels that can cause a halt
  are channels that have the update of this payment irrevocably committed. This
  only happens when both sides send a \textsc{revokeAndAck} that updates the
  channel from a version that contains the relevant HTLC to a version that
  doesn't; and when an honest party receives such a \textsc{revokeAndAck}
  message, it logs the update in \texttt{updatesToReport}
  (Fig.~\ref{alg:protocol:pay:revokeAndAck},
  ...
  halts on line~\ref{alg:fpaynet:resolvepay:update:halt} of
