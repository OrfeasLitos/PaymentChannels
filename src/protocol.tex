    ...
    \environment}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \State choose unique temporary ID \textit{tid} \Comment{unique for the
          two parties}
  ...
  \begin{figure}[H]
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ - \textsc{checkNew}}
      \begin{algorithmic}[1]
        ...
        \State Upon receiving (\textsc{checkNew}, \alice, \bob, $x$,
        \textit{tid}) from \environment: \Comment{new message: represents lnd
        polling daemon}
        \Indent
    ...
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ - invoice}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \mathtt{RHSDelay}$ \TODO{\alice{} doesn't know these Bob, RHS delays}
    ...
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ - \textsc{updateFulfillHtlc}}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \If{we have a channel $\mathtt{phcid}'$ that has a received HTLC with
          hash $h$ with number $\mathtt{HTLCNo}'$} \Comment{We are intermediary}
            \If{$\mathtt{HTLCNo}' \leq \mathtt{lastRemoteSigned}'$}
            \Comment{HTLC committed}
              \State send (\textsc{read}) to \ledger{} and assign reply to
              $\Sigma_{\alice}$
    ...
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ - \textsc{revokeAndAck}}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \mathtt{CombinePubKey}\left(ptb_{\mathrm{rev}}, ph_{\mathrm{com},
          n+2}\right)$
          \State \TODO{Change simple labels to keypairs}
    ...
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ - \textsc{push}}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \State send (\textsc{updateFulfillHtlc}, \textit{pchid}, HTLC\_no,
          $R$) to \textit{pchid} counterparty
        \EndIndent
      \end{algorithmic}
    \end{protocolbox}
    \caption{}
