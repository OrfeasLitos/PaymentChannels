      ...
      \begin{algorithmic}[1]
        ...
        \Indent
          \ifelseieee{
            ...
            \State $\mathtt{unclaimedOfferedHTLCs}, \mathtt{gotPaid} \gets
            \emptyset$
    ...
    \environment}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          Their use reduces from three to one the public keys that have to be
          exchanged on each update.}
    ...
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ -- poll}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \texttt{updatesToReport}, \texttt{paymentsToReport} and send them to
          \environment{} with message name \textsc{news}
    ...
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ -- invoice}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \State remove mark from, add $h$ and \texttt{HTLCNo} to entry in
          \texttt{pendingPay}
    ...
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ -- \textsc{revokeAndAck}}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \EndIf
          \State $\mathtt{updateDiffs}(\mathtt{channel}) \gets \emptyset$
      ...
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \Comment{counterparty cooperative}
            \If{$\exists (\_, R, \mathit{expayid}, \mathit{payid}, x) \in
            \mathtt{gotPaid}$} \Comment{receiver}
              \State add ($\mathit{expayid}, x$, reference to
    ...
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ -- \textsc{fulfillOnChain}}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \ForAll{channels}
            \If{there exist HTLCs with hashes $h_1, \dots, h_k$ in latest
            $\mathtt{localCom}_n$ for which we have sent both
            \textsc{updateFulfillHtlc} and \textsc{commitmentSigned} to a
            transaction without that HTLC to counterparty, but have not received
            the corresponding \textsc{revokeAndAck} AND the HTLC expires within
            $\left[t, t + \tochain{}\right]$}
              ...
              \texttt{toSubmit}
              \ForAll{such HTLCs for which we are the payee (i.e., we have
              received a (\textsc{sendInvoice}, $x$, \textit{expayid}, \_)
              message in response to which we generated and sent $h_i$)}
                \State add (\textit{expayid}, $x$) to \texttt{paymentsToReport}
    ...
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ -- close unilaterally}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          $i$}
            \If{there is a (\_, $R$, \textit{expayid}, \_, $x$) entry in
            \texttt{gotPaid} for $R$ the preimage of $\mathrm{HTLC}_i$}
              \State remove entry from \texttt{gotPaid}
              \State add (\textit{expayid}, $x$) to \texttt{paymentsToReport}
            \EndIf
      ...
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \If{\texttt{channel} is not marked ``coop closing''}
            \State stop serving any (\textsc{pay}, \textsc{closeChannel})
            message from \environment{} and any (\textsc{UpdateAddHtlc},
            \textsc{sendInvoice}) message from counterparty for this channel.
