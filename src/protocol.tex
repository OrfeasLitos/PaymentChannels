\section{Lightning Protocol}
      ...
      \begin{algorithmic}[1]
        ...
        \Indent
          \ifelseieee{
            ...
            \emptyset$
            \State $\mathtt{paymentsToReport} \gets \emptyset$
            \State $\mathtt{unclaimedOfferedHTLCs}, \mathtt{gotPaid}
            \gets \emptyset$
            ...
            \mathtt{paymentsToReport} \gets \emptyset$
            \State $\mathtt{newChannels}, \mathtt{closedChannels},
            \mathtt{updatesToReport}, \mathtt{gotPaid} \gets \emptyset$
        ...
        \Indent
          \If{we haven't received (\textsc{register}) and (\textsc{toppedUp})
    ...
    \environment}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \State \Comment{Keys marked with ``$b$'' are called basepoint keys.
          Their use reduces the number of public keys that have to be exchanged
          on each update from three to one.}
    ...
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ -- poll}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \texttt{updatesToReport}, \texttt{paymentsToReport} and send
  ...
  \begin{figure}[H]
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ -- pay}
      \begin{algorithmic}[1]
        ...
        $\overrightarrow{\mathtt{path}}$, \textit{expayid}) from
        \environment:
        \Indent
          ...
          \texttt{pendingPay}
          \State send (\textsc{sendInvoice}, $x$, \textit{expayid},
          \textit{payid}) to \bob{}
        ...
        \Indent
          \State ensure there is no (\bob, \_, \textit{expayid},
          \textit{payid}, \_) entry in \texttt{pendingGetPaid}
          \State choose random, unique preimage $R$
          \State add (\bob, $R$, \textit{expayid}, \textit{payid},
          $x$) to \texttt{pendingGetPaid}
    ...
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ -- invoice}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \State remove mark from, add $h$ and \texttt{HTLCNo} to entry
          in \texttt{pendingPay}
    ...
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ -- \textsc{updateAddHtlc}}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          ($pt_{\mathrm{htlc}, n+1}$, \texttt{IncomingCltvExpiry}
          absolute) $\vee$ ($pt_{\mathrm{htlc}, n+1}$ $\wedge$
          $ph_{\mathrm{htlc}, n+1}$, on preimage of $h$)) htlc output (all with
          $n+1$ keys) to \texttt{localCom}, marked with \texttt{HTLCNo}
          ...
          hop}
            ...
            ph'_{\mathrm{rev}, n+1} \vee
            \left(ph'_{\mathrm{htlc}, n+1} \wedge
            pt'_{\mathrm{htlc}, n+1}, \text{ on
            preimage}\right.\right.$ $\left.\left.\text{of } h\right) \vee
            ph'_{\mathrm{htlc}, n+1}, \mathtt{OutgoingCltvExpiry}
            ...
            \State add an additional ($x$, $pt'_{\mathrm{rev}, n+1}$
            $\vee$ ($pt'_{\mathrm{htlc}, n+1}$, on preimage of $h$)
            $\vee$ ($pt'_{\mathrm{htlc}, n+1}$ $\wedge$
            $ph'_{\mathrm{htlc}, n+1},$ \texttt{OutgoingCltvExpiry}
            absolute)) output (all with $n+1$ keys) to
            $\mathtt{localCom}'$, marked with $\texttt{HTLCNo}'$
    ...
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ -- \textsc{revokeAndAck}}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \State append \texttt{receipt}(\texttt{channel}) to
          \texttt{updatesToReport}
          \label{alg:protocol:pay:raa:report:updates}
          \ForAll{$(\_, \_, \textit{payid}, \mathtt{ref}) \in
          ...
          \EndIf
          \State $\mathtt{updateDiffs}(\mathtt{channel}) \gets
          \emptyset$
      ...
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \Comment{counterparty cooperative}
            \If{$\exists (\_, R, \mathit{expayid}, \mathit{payid}, x)
  ...
  \begin{figure}[H]
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ -- \textsc{fulfillOnChain}}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \ForAll{channels}
            \If{there exist HTLCs with hashes $h_1, \dots, h_k$ in
            latest $\mathtt{localCom}_n$ for which we have sent both
            \textsc{updateFulfillHtlc} and \textsc{commitmentSigned} to a
            transaction without those HTLCs to counterparty, but have
            not received the corresponding \textsc{revokeAndAck}s AND
            some of the HTLCs expire within $\left[t, t +
            \tochain{}\right]$}
              ...
              \texttt{toSubmit}
              \ForAll{such HTLCs for which we are the payee (i.e., we
    ...
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ -- close unilaterally}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          $i$}
            \If{there is a (\_, $R$, \textit{expayid}, \textit{payid},
    ...
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ -- close cooperatively}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          from \environment{} and any (\textsc{UpdateAddHtlc},
        ...
        \Indent
          ...
          \If{\texttt{channel} is not marked ``coop closing''}
            \State stop serving any (\textsc{pay},
