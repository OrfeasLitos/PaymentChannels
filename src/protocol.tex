\ \\ \noindent {$\Pi_{\mathrm{LN}}$} (self is \alice)
  ...
  \begin{algorithmic}[1]
    ...
    \Indent
      ...
      $\overrightarrow{\mathtt{path}}$ $\left(x, \mathtt{delay}_i\right)$ pairs,
      where the last \texttt{delay} is $t + 2k + 1$ and every previous
      \texttt{delay} is incremented by $3k$
      ...
      ph_{\mathrm{htlc}, n+1}, \text{largest } \mathtt{delay} \text{
      ...
      largest \texttt{delay}, $\left(\mu_0, \delta_0\right)$) to \textit{pchid}
      channel counterparty and \adversary
    \EndIndent
    \State

    \State Upon receiving (\textsc{updateAddHtlc}, \textit{pchid},
    $\mathtt{HTLCNo}, x, h, \mathtt{delay}, M$) from \bob:
    \Indent
      ...
      \mathtt{delay} \text{ absolute}\right) \vee\right.$
      ...
      \ElsIf{$\delta \neq \mathtt{receiver}$} \Comment{Send HTLC to next hop}
        ...
        \left(ph_{\mathrm{htlc}, n+1} \wedge pt_{\mathrm{htlc}, n+1}, \text{ on
        preimage}\right.\right.$ $\left.\left.\text{of } h\right) \vee
        ph_{\mathrm{htlc}, n+1}\mathtt{delay}' \text{ absolute}\right)$ output
        (all with $n+1$ keys) to \texttt{remoteCom}, marked with \texttt{HTLCNo}
        ...
        $x, h, \mathtt{delay}', M$) to $\mathit{pchid}'$ counterparty
      \EndIf
    \EndIndent
    \State
