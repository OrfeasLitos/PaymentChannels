\section{Lightning Protocol}
  ...
  public key with some initial funds and the non-serving any other messages
    ...
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ - \textsc{revokeAndAck}}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          (Figs.~\ref{alg:protocol:close:unilateral}
          or~\ref{alg:protocol:close:coop}) has been initiated}
            \State continue execution at
            Fig.~\ref{alg:protocol:close:unilateral},
            l.~\ref{alg:protocol:close:unilateral:init:continue} or
            Fig.~\ref{alg:protocol:close:coop},
            l.~\ref{alg:protocol:close:unilateral:shutdown:continue}
            respectively
          \EndIf
        \EndIndent
      \end{algorithmic}
