    ...
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ (self is \alice{} always) - support}
      \begin{algorithmic}[1]
        ...
        \Function{GetKeys}{}
          ...
          \textsc{MasterKeyGen}\left(\right)$ \Comment{For com output to remote}
          \State $\left(p_{\mathrm{dpay}}, s_{\mathrm{dpay}}\right) \gets
          \textsc{MasterKeyGen}\left(\right)$ \Comment{For com output to self}
          \State $\left(p_{\mathrm{htlc}}, s_{\mathrm{htlc}}\right) \gets
          \textsc{MasterKeyGen}\left(\right)$ \Comment{For htlc output to self}
