\section{Lightning Protocol}
  ...
  \begin{figure}[H]
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ - \textsc{checkNew}} \TODO{continue}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \State replace ``broadcast'' mark in \texttt{channel} with ``no
          \textsc{fundingLocked}''
          \State mark \texttt{channel} with ``\textsc{fundingLocked} sent''
...
%            \label{alg:protocol:checkNew:report}
%          \EndIf
  ...
  \begin{figure}[H]
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ - \textsc{fundingLocked}}
      \begin{algorithmic}[1]
        ...
        $pt_{\mathrm{com}, 2}$) from \bob: \TODO{continue}
        \Indent
