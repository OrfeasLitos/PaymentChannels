\section{Lightning Protocol}
  \TODO{find out what \tochain{} (\textbackslash{}tochain) must be}
  \begin{figure}[H] % [!htbp]
    ...
    \environment}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \mathtt{relayDelay}, ph_F, phb_{\mathrm{pay}}, phb_{\mathrm{dpay}},
    ...
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ - invoice}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \textit{pchid}s and \texttt{CltvExpiryDelta}s \Comment{Payment
          completes only if $\forall i \in \overrightarrow{\mathtt{path}},
          \mathtt{CltvExpiryDelta}_i \geq 3k + \mathtt{RelayDelay}_i$}
    ...
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ - \textsc{push}}
      \begin{algorithmic}[1]
        ...
        \Indent
          \State $\mathtt{toSubmit} \gets \emptyset$
          \ForAll{channels}
            \If{we have sent \textsc{updateFulfillHtlc} to counterparty and have
            sent \textsc{commitmentSigned} to a transaction without that HTLC,
            but have not received the corresponding \textsc{revokeAndAck}}
